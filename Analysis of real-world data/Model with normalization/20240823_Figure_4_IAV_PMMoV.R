...
data <- data_FL
data_IAV <- data %>% select(collection_date, Count_IAV, Positive_IAV, Concentration_IAV, Concentration_sub_IAV, Concentration_PMMoV)
colnames(data_IAV) <- c("date", "count", "positive", "concentration", "substitution", "PMMoV")
...
print(1-nrow(data_ww)/nrow(data_row_CD))
...
PMMoV_ave <- mean(data_IAV$PMMoV, na.rm = TRUE)

data_list_ww <- list(n_all = sample_size, n_cd = sample_size_CD, n_d = sample_size_D,
                     ww = ww, nd = ND, row_cd = data_row_CD$true, row_d = data_row_D$true, n_count = count, ww_ind = PMMoV, mu_ind_ave = PMMoV_ave)

rstan_options(auto_write = TRUE)
options(mc.cores = parallel::detectCores())

mcmc <- stan(
  file = "2025026_state_space_real_data_PMMoV.stan", 
  data = data_list_ww,
  seed = 1,
  chain = 6,
  iter = 10000,
...
print(mcmc, pars = c("s1_ind", "s2_ind", "c1", "c2", "s1", "s2"), probe = c(0.025, 0.50, 0.975))
...
state_name <- "mu_ind"
result_7_ind <- data.frame(t(apply(
  X = mcmc_sample[[state_name]],
  MARGIN = 2,
  FUN = quantile,
  probs = c(0.025, 0.5, 0.975)
...
colnames(result_7_ind) <- c("low_7_ind", "med_7_ind", "upr_7_ind")


data_concentration <- cbind(data_IAV, result_7_a, result_7_ind)

write.csv(x = data_concentration, file = "/home/u31/hirokiando/20250226_IAV_CA.csv")


#data_ef <- data.frame(mcmc_sample["mu"])
#write.csv(x = data_ef, file = "/home/u31/hirokiando/20250226_mcmc_IAV_CA.csv")
...
write.csv(x = data_eff_fig, file = "/home/u31/hirokiando/20250226_IAV_CA_Re.csv")
...
write.csv(x = data_fig_logis, file = "/home/u31/hirokiando/20250226_IAV_CA_logistic.csv")
