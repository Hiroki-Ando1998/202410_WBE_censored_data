...
ww_PMMoV <- data_ww$PMMoV
print(1-nrow(data_ww)/nrow(data_row_CD))

data_nd <- data_stan %>% filter(count > 0)
ND <- data_nd$positive
count <- data_nd$count
ww_PMMoV_2 <- data_nd$PMMoV

PMMoV_ave <- mean(data_IAV$PMMoV, na.rm = TRUE)

data_list_ww <- list(n_all = sample_size, n_cd = sample_size_CD, n_d = sample_size_D,
                     ww = ww, nd = ND, row_cd = data_row_CD$true, row_d = data_row_D$true, n_count = count, ww_PMMoV = ww_PMMoV,  ww_ind = ww_PMMoV_2, mu_ind_ave = PMMoV_ave)

rstan_options(auto_write = TRUE)
options(mc.cores = parallel::detectCores())

mcmc <- stan(
  file = "2025026_state_space_real_data_PMMoV_eq(2).stan", 
  data = data_list_ww,
  seed = 1,
  chain = 6,
  iter = 10000,
...
write.csv(x = data_concentration, file = "/home/u31/hirokiando/20250303_IAV_FL.csv")


data_ef <- data.frame(mcmc_sample["mu"])
#write.csv(x = data_ef, file = "/home/u31/hirokiando/20250303_mcmc_IAV_FL.csv")
...
write.csv(x = data_eff_fig, file = "/home/u31/hirokiando/20250303_IAV_FL_Re.csv")
...
write.csv(x = data_fig_logis, file = "/home/u31/hirokiando/20250303_IAV_FL_logistic.csv")
