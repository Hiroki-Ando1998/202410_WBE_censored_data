...
data_row_D <- data.frame(true = which(!is.na(data_stan$substitution))) #pick row numbers for censored data
sample_size_D <- nrow(data_row_D)

#vector of concentration and positive data for the analysis
data_1 <- data_stan %>% drop_na(substitution)
ww <- data_1$substitution

data_list_ww <- list(n_all = sample_size, n_d = sample_size_D,
                     ww = ww, row_d = data_row_D$true)

rstan_options(auto_write = TRUE)
options(mc.cores = parallel::detectCores())

mcmc <- stan(
  file = "20240809_stat_space_real_substitution_data.stan", 
  data = data_list_ww,
  seed = 1,
  chain = 4,
  iter = 200000,
...
print(mcmc, pars = c("s1", "s2"), probe = c(0.025, 0.50, 0.975))
...
write.csv(x = data_concentration, file = "/home/u31/hirokiando/20240901_IAV_CA.csv")


data_ef <- data.frame(mcmc_sample["mu"])
#write.csv(x = data_ef, file = "/home/u31/hirokiando/20240606_mcmc_IAV_CA.csv")
...
write.csv(x = data_eff_fig, file = "/home/u31/hirokiando/20240901_IAV_CA_Re.csv")
