...
#Figure 2c
data_c <- read.csv("figure_2_c.csv")

plot <- ggplot(data_c, aes(x = as.factor(sampling_number), y = error, fill = as.factor(sampling_number)))
plot <- plot + geom_dotplot(binaxis="y", binwidth=0.003, stackdir = "center", alpha = 1.0) 
...
plot <- plot + scale_y_continuous(limits = c(0,0.15), breaks = seq(0, 0.15, 0.03))
...
#Figure2_d
data_d <- read.csv("figure_2_d.csv")
...
#Figure2_e
data_e <- read.csv("figure_2_e.csv")
