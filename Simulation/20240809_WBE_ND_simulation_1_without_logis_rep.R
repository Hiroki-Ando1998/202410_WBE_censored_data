...
size <- 10 #the number of analyzed samples
p3 <- c()
...
data_sim_final_6 <- data_sim_final_5[20:250,]
...
data_row_D <- data.frame(detected =  which(!is.na(data_stan$ND))) #pick row numbers of detected data
sample_size_D <- nrow(data_row_D)

data_ww <- data_stan %>% drop_na()
ww <- data_ww$logWW_rep #logWW_rep
...
print(mcmc_7, pars = c("s1", "s2"), probe = c(0.025, 0.50, 0.975))
...
print(mcmc_3, pars = c("s1", "s2"), probe = c(0.025, 0.50, 0.975))
...
print(mcmc_1, pars = c("s1", "s2"), probe = c(0.025, 0.50, 0.975))
